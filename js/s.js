(function (w, d) {
    ...
    zaraz.track = async function (cL, cM, cN) {
        return new Promise(((cO, cP) => {
            ...
            for (const cR of [localStorage, sessionStorage]) Object.keys(cR || {}).filter((cT => cT.startsWith("_zaraz_"))).forEach((cS => {
                try {
            ...
            Object.keys(zaraz.pageVariables).forEach((cU => cQ.data[cU] = JSON.parse(zaraz.pageVariables[cU])));
            Object.keys(zaraz.__zcl).forEach((cV => cQ.data[`__zcl_${cV}`] = zaraz.__zcl[cV]));
...
//
    ...
    zaraz.set = function (cY, cZ, c$) {
        ...
        if (void 0 !== cZ) {
            c$ && "session" == c$.scope ? sessionStorage && sessionStorage.setItem(prefixedKey, cZ) : c$ && "page" == c$.scope ? zaraz.pageVariables[cY] = cZ : localStorage && localStorage.setItem(prefixedKey, cZ);
            zaraz.__watchVar = {key: cY, value: cZ}
        }
    };
    for (const {m: db, a: dc} of zarazData.q.filter((({m: dd}) => ["debug", "set"].includes(dd)))) zaraz[db](...dc);
    ...
    zaraz.fulfilTrigger = function (bY, bZ, b$, ca) {
        ...
        zaraz.track("__zarazEmpty", {...b$, __zarazClientTriggers: zaraz.__zarazTriggerMap[bY]}, ca)
    };
    window.dataLayer = w.dataLayer || [];
    zaraz._processDataLayer = dI => {
        for (const dJ of Object.entries(dI)) zaraz.set(dJ[0], dJ[1], {scope: "page"});
        if (dI.event) {
            if (zarazData.dataLayerIgnore && zarazData.dataLayerIgnore.includes(dI.event)) return;
            let dK = {};
            for (let dL of dataLayer.slice(0, dataLayer.indexOf(dI) + 1)) dK = {...dK, ...dL};
            delete dK.event;
    ...
    zaraz.__zcl.track = !0;
    zaraz._p({"e": ["(function(w,d){w.zarazData.executed.push(\"Pageview\");})(window,document)"]})
})(window, document);